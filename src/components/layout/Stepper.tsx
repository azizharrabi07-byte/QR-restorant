...
import { OnboardingStep, StepDefinition } from "../../types";
...
export const STEPS: StepDefinition[] = [
  {
    ...
    shortTitle: "Identity",
    description: "Name, URL slug & brand emblem",
  },
  {
    step: 2,
  ...
  },
  {
    ...
    shortTitle: "Products",
    description: "Items, prices, dietary tags & images",
  },
  {
    step: 5,
  ...
  },
  {
    ...
    shortTitle: "Tables",
    description: "Grid of scannable table QR cards",
  },
];
...
export function Stepper({ currentStep, onSelectStep, primaryColor }: StepperProps) {
  const getStepIcon = (step: OnboardingStep, isCompleted: boolean) => {
    if (isCompleted) {
    ...
    <div className="w-full border-b border-white/10 bg-[#080808]">
      <div className="max-w-7xl mx-auto px-4 sm:px-8">
        <nav aria-label="Onboarding Progress" className="overflow-x-auto no-scrollbar py-4">
          <ol className="flex items-center min-w-max md:min-w-0 md:grid md:grid-cols-6 gap-2 sm:gap-4">
            {STEPS.map((s, idx) => {
              ...
              return (
                <li key={s.step} className="relative">
                  ...
                  >
                    ...
                    {/* Connecting line indicator for next item on desktop */}
                    {idx < STEPS.length - 1 && (
